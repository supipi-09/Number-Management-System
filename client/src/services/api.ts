...
import { User, NumberRecord, NumberLog, DashboardStats, LoginCredentials, ApiResponse } from '../types';
...
export const numbersAPI = {
  ...
  updateNumber: async (id: string, data: Partial<NumberRecord>): Promise<ApiResponse<NumberRecord>> => {
    const response = await api.put(`/numbers/${id}`, data);
    return response.data;
  },

  createNumber: async (data: Partial<NumberRecord>): Promise<ApiResponse<NumberRecord>> => {
    const response = await api.post('/numbers', data);
    return response.data;
  },

  importNumbers: async (file: File): Promise<ApiResponse<{ success: number; failed: number; errors: string[] }>> => {
